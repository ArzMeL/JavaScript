Vue.component('header-comp',{
    ...
    template: ` <div class="head">
                    <h1><a class="main-link" href="index.html">Mobishop</a></h1>
...
Vue.component('cart',{
    ...
    <div class="cart_content">
        ...
        <div class="cart" v-if='isVisibleCart'>
            <p v-if='!cartitems[1].length'>Корзина пуста</p>
            <div class='cart_item' v-for="item of cartitems[1]" :key='item.id_product'>
                ...
                <div class='cart_item_info'>
                    ...
                    <p type='number' class='cart_item_count' @input='$parent.$emit("addproduct",item)'>Количество: {{ item.quantity }} шт.</p>
                    <p class='cart_item_price'>Цена: {{item.quantity * item.price}} руб.</p>
                </div>
                <button @click='deleteItem(item.id_product)'>&times</button>
            </div>
            <p v-if='cartitems[1].length'>Общая стоимость: {{calculateCart()}} руб.</p>
        </div>
    </div>
    `
})
